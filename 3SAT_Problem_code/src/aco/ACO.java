...
public class ACO {
	...
	public static void runACOAlgo() {
		for (int x = 1; x <= NUMBER_OF_INSTENCES; x++) { // pour les 100 instnaces 
			...
			sat = Load.LoadSat("/res/uf75-325/uf75-0"+x+".cnf"); // chargement de l'instance
	 
	        pheromon = new double[Load.VAR_NUM][2]; // declaration de la matrice du pheromone
			...
			while (iter < MAX_ITERATIONS && fitness != 325 &&  System.currentTimeMillis() - startTime < DEFAULT_TIMEOUT) {
	            ...
	            for (int j = 0; j < Load.VAR_NUM; j++) { // offline update
            ...
            float accuracy = (float)fitness/Load.CLAUSE_NUMBER*100;
